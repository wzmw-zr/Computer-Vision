...
**白化(Whitened)**：如果输入被白化过了，网络训练拟合地更快，比如线性变换到0均值，单位方差，不相关。通过白化每层的输入，我们能够获得固定分布的输入，这样可以消除ICS的不良影响。
...
使用**滑动平均数**(moving averages)，我们能够**追踪模型训练时的准确率**。
...
注意，因为我们需要正则化$Wu+b$，因为后面的**减均值操作**，**偏置$b$是可以省略的**，那么：
$$
z=g(\mathrm{BN}(Wu))
$$
